...
import { getImobilBySlug, getImobilPosts } from "@/lib/imobil"
...
export default async function InvestmentDetailsPage({ params }: Props) {
    ...
    const { content, data } = await getImobilBySlug(slug)
...
export function generateStaticParams() {
    return getImobilPosts().map((p) => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: Props) {
    const { slug } = await params
    const { data } = await getImobilBySlug(slug)
    return {
        title: data?.title
            ? `${data.title} | Procurarea locuinței`
            : "Procurarea locuinței",
        description: data?.excerpt ?? "Imobil details",
    }
}
