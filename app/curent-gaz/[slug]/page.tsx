...
import { getCurentGasBySlug, getCurentGasPosts } from "@/lib/curentgas"
...
export default async function InvestmentDetailsPage({ params }: Props) {
    ...
    const { content, data } = await getCurentGasBySlug(slug)
...
export function generateStaticParams() {
    return getCurentGasPosts().map((p) => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: Props) {
    const { slug } = await params
    const { data } = await getCurentGasBySlug(slug)
    return {
        title: data?.title ? `${data.title} | Curent/Gaz` : "Curent/Gaz",
        description: data?.excerpt ?? "Curent/Gaz details",
    }
}
